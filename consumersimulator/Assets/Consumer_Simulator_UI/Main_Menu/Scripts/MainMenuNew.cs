	...
	public class MainMenuNew : MonoBehaviour {
		...
		IEnumerator LoadAsynchronously(string sceneName)
		{
            ...
            while (!operation.isDone)
            {
                ...
                {
                    Debug.Log( "operation" + operation.progress );
                    operation.allowSceneActivation = true;
				}
                yield return null;
            }
        }
   //     IEnumerator FadeLoadingScreenCo(float targetValue, float duration)
   //     {
			//float stValue = canvasGroup.alpha;
			//float thisTime = 0;
   //         while (thisTime < duration)
   //         {
			//	canvasGroup.alpha = Mathf.Lerp( stValue , targetValue , thisTime / duration );
